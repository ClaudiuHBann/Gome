...
int main()
{
    ...
    server->Start([](auto client) {
        ...
        client->Receive([client](auto, auto tuple) {
            ...
            client->Send(get<2>(tuple), get<1>(tuple), [](auto, auto) { TRACE(TEXT("Server: Sent data.")); });
        });
    });
    ...
    auto &&endpoints =
        resolver.resolve(ToStringType<char>(String(TEXT("127.0.0.1"))), ToStringType<char>(ToString(6969)));
    client.Connect(endpoints, [&](auto, auto) {
        TRACE(TEXT("Client: Connected."));
        client.Send({byte('6'), byte('9')}, HeaderMetadata::Type::PING, [&](auto, auto) {
            TRACE(TEXT("Client: Sent."));
            client.Receive([](auto, auto tuple) { TRACE(TEXT("Client: Received.")); });
        });
    });
