  ...
  public:
    ...
    template <typename ForwardIterator> explicit uuid(ForwardIterator first, ForwardIterator last)
    {
        if (std::distance(first, last) == 16)
            std::transform(first, last, std::begin(data), [](auto b) { return (value_type)b; });
    }
...
class uuid_system_generator
{
  public:
    ...
    uuid operator()()
    {
        ...
        GUID newId;
        HRESULT hr = UuidCreate(&newId);

        if (FAILED(hr))
        {
            throw std::system_error(hr, std::system_category(), "UuidCreate failed");
        }
